...
#    mDCO    = mCO+2.014               # - HCO molecular weight
    ...
    '''def __init__(self,params=[-0.5,0.09,1.,10.,1000.,150.,51.5,2.3,1e-4,0.01,33.9,19.,69.3,
                 [.79,1000],[10.,1000],-1, 0.1, 30, 0.15],obs=[180,131,300,170],rtg=True,vcs=True,
                 exp_temp=False,line='co',ring=None,include_selfgrav=False):'''
    def __init__(self,q=-0.5,McoG=0.09,pp=1.,Rin=1.,Rout=1000,Rc=150.,incl=51.5, Mstar=2.3,Xco=1e-4,vturb=0.01,Zq0=33.9,Tmid0=19,Tatm0=69.3,sigbound=[.79,1000], Rabund=[10,1000],handed=-1,wind_speed=0.1,wind_angle=30,min_wind_height=0.15,vtalpha=0,vtsig=0, nr=180,nphi=131,nz=300,zmax=170,rtg=True,vcs=True, exp_temp=False,line='co',ring=None,include_selfgrav=False):

        ### Changed the init function so that it is easier to specify a subset of the parameters
        ### while keeping the rest as defaults.
        params = [q,McoG,pp,Rin,Rout,Rc,incl,Mstar,Xco,vturb,Zq0,Tmid0,Tatm0,sigbound,Rabund,handed,wind_speed,wind_angle,min_wind_height,vtalpha,vtsig]
        obs = [nr,nphi,nz,zmax]
    ...
    def set_params(self,params):
        ...
        self.handed = params[15]            #Handedness of Disk
        self.costhet = np.cos(self.thet)    # - cos(i)
        self.sinthet = np.sin(self.thet)    # - sin(i)
        self.wind_speed = params[16]             # - Velocity of Wind (if any)
        self.wind_angle = math.radians(params[17])    # - Angle of Wind
        self.min_wind_height = params[18]             # - Minimum height of wind as a proportion of radius
    ...
    def set_incl(self, inc):
        self.thet = math.radians(inc)

    def set_wind_speed(self, v):
        self.wind_speed = v

    def set_wind_angle(self, a):
        self.wind_angle = math.radians(a)

    def set_min_wind_height(self, h):
        self.min_wind_height = h
    ...
    def set_rt_grid(self,vcs=True):
        ...
        # Copies phi, each with length nphi, into each coulmn of a nphi*nr*nz matrix
        angles = np.tile(np.sin(phi)[:, np.newaxis, np.newaxis], (1, self.nr, self.nz))

        rz = np.zeros((self.nz, self.nr))
        for z in range(self.nz):
            for r in range(self.nr):
                if (self.zmax*np.abs(self.nz/2 -0.5 - z) >= self.Rout*self.min_wind_height*np.abs(self.nr/2 -0.5 - r)):
                    rz[z, r] = 1
        heightcutoff = (np.tile(rz.T, (self.nphi, 1, 1)))        

        # arrays in [phi,r,s]
        X = (np.outer(R,np.cos(phi))).transpose()
        Y = (np.outer(R,np.sin(phi))).transpose()

        #duplicates array into an nphi x nr x nz array
        ...
        cs = np.sqrt(2*self.kB/(self.Da*2)*tT)
        vwind = self.wind_speed*cs*(np.sign(tdiskZ)*np.cos(self.thet)*np.cos(self.wind_angle) + np.sin(self.thet)*np.sin(self.wind_angle)*angles)*heightcutoff
        ...
        self.Smid = Smid
        self.phi = phi
        self.X3 = np.tile(X[:, :, np.newaxis], (1, 1, self.nz))
        #self.r = tr
        self.T = tT
        self.dBV = tdBV
        self.rhoG = trhoG
    ...
    def get_params(self):
        ...
        params.append(self.wind_speed)
        params.append(math.degrees(self.wind_angle))
        params.append(self.min_wind_height)
        return params
